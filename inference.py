...
parser.add_argument('--checkpoint_path', type=str,
                    help='Name of saved checkpoint to load weights from', required=False, default='./checkpoints/wav2lip_gan.pth')

parser.add_argument('--face', type=str,
                    help='Filepath of video/image that contains faces to use', required=False, default='./emma_304x300.mp4')
parser.add_argument('--audio', type=str,
                    help='Filepath of video/audio file to use as raw audio source', required=False, default='./tg.wav')
parser.add_argument('--outfile', type=str, help='Video path to save result. See default for an e.g.',
                    default='output/result_voice.mp4')
...
def face_detect_with_cache(images, input_video_fname, input_file_size):
    global device
    ...
    if args.box[0] == -1:
        if not args.static:
            ...
            face_det_results = face_detect_with_cache(
                frames,
                os.path.basename(args.face),
                os.stat(args.face).st_size,
            )
        else:
            face_det_results = face_detect_with_cache(
                [frames[0]],
...
def main():
    if not os.path.isfile(args.face):
        ...
        while 1:
            ...
            if args.rotate:
                frame = cv2.rotate(frame, cv2.cv2.ROTATE_90_CLOCKWISE)  # type: ignore
    ...
    for i, (img_batch, mel_batch, frames, coords) in enumerate(tqdm(gen,
        ...
        if i == 0:
            ...
            out = cv2.VideoWriter('temp/result.avi',
                                  cv2.VideoWriter_fourcc(*'DIVX'), fps, (frame_w, frame_h))  # type: ignore
