...
full_frames_by_file = {}    #
# start frame to play from current video, to make smooth chunks playing
...
parser.add_argument('--wav2lip_batch_size', type=int,
                    help='Batch size for Wav2Lip model(s)', default=1024)
...
def face_detect(images, args):
    ...
    detector = face_detection.FaceAlignment(
        face_detection.LandmarksType._2D, flip_input=False, device='cuda:0' if device == 'cuda' else device)
        ...
        try:
            for i in tqdm(range(0, len(images), face_det_batch_size)):
                predictions.extend(detector.get_detections_for_batch(
                    np.array(images[i:i + face_det_batch_size])))
        except RuntimeError:
...
def face_detect_with_cache(images, args, input_video_fname, input_file_size):
    ...
    if not Path(cache_path).exists():
        print("Face detection cache "+cache_filename +
              " doesn't exist, calling face detect using device: "+device)
        print("***")
...
def datagen(frames, mels, args):
    ...
    if len(frames) > 60:
        ...
        if start_frame < 0 or start_frame >= len(frames):
            print(" -- start_frame was " +
                  str(start_frame)+", setting it back to 0")
            start_frame = 0
        ...
        if start_frame_global >= len(frames):
            ...
            print(" -- start_frame_global was >= frames, setting it back to: " +
                  str(start_frame_global))
    else:
        start_frame = 0
    ...
    if args.box[0] == -1:
        if not args.static:
            face_det_results = face_detect_with_cache(frames, args, os.path.basename(
                args.face), os.stat(args.face).st_size)  # BGR2RGB for CNN face detection
        else:
            face_det_results = face_detect_with_cache(
                [frames[0]], args, os.path.basename(args.face), os.stat(args.face).st_size)
    else:
    ...
    print(str(time.time())+" after face detection, mels: "+str(len(mels))+", frames: " +
          str(len(frames))+", faces: "+str(len(face_det_results))+", start_frame: "+str(start_frame))
...
def load_model(path, device="cpu"):
    if os.path.isfile(path) == False:
        ...
        if checkpoint_fname == "wav2lip.pth":
            print(f"Downloading "+checkpoint_fname +
                  " model from huggingface into "+checkpoint_dir+"...")
            hf_hub_download(repo_id="Ftfyhh/wav2lip", filename="wav2lip.pth",
                            local_dir=checkpoint_dir, local_dir_use_symlinks=False)
        elif checkpoint_fname == "wav2lip_gan.pth":
            print(f"Downloading "+checkpoint_fname +
                  " model from huggingface into "+checkpoint_dir+"...")
            hf_hub_download(repo_id="Ftfyhh/wav2lip", filename="wav2lip_gan.pth",
                            local_dir=checkpoint_dir, local_dir_use_symlinks=False)
        else:
            print(
                "Error: Download your model manually and put into /wav2lip/checkpoints/")
...
def wav2lip_main(args):  # noqa:C901
    ...
    if not os.path.isfile(args.face):
        ...
        full_frames = [cv2.imdecode(np.fromfile(
            args.face, np.uint8), cv2.IMREAD_UNCHANGED)]
        fps = args.fps
    ...
    if (not len(wav)):
        print("out_"+str(args.chunk)+".wav len " +
              str(len(wav))+" is bad, exiting")
        return
    ...
    for i, (img_batch, mel_batch, frames, coords) in enumerate(tqdm(gen,
        ...
        if i == 0:
            ...
            out = cv2.VideoWriter('modules/wav2lip/temp/result_'+str(args.chunk)+'_tmp.mp4',
                                  cv2.VideoWriter_fourcc(*'avc1'), fps, (frame_w, frame_h))  # type: ignore # 0.00s test mp2v avc1
        ...
        if (len(mel_batch) < batch_size):
            ...
            if padding_len:
                ...
                # Add an extra dimension to allow concatenation
                last_img = img_batch[-1][np.newaxis, ...]
                img_padding = np.repeat(last_img, padding_len, axis=0)
                img_batch = np.concatenate((img_batch, img_padding), axis=0)
                # Add an extra dimension to allow concatenation
                last_mel = mel_batch[-1][np.newaxis, ...]
        ...
        img_batch = torch.FloatTensor(np.transpose(
            img_batch, (0, 3, 1, 2))).to(device)  # 0.001 s
        mel_batch = torch.FloatTensor(np.transpose(
            mel_batch, (0, 3, 1, 2))).to(device)  # 0.001 s
        ...
        try:
            with torch.no_grad():
                # first run 0.14 s, other runs: 0.02 s
                pred = model(mel_batch, img_batch)  # type: ignore
            ...
            out.release()
            os.replace('modules/wav2lip/temp/result_'+str(args.chunk)+'_tmp.mp4',
                       'modules/wav2lip/temp/result_'+str(args.chunk)+'.mp4')
            torch.cuda.empty_cache()
            return -1
        ...
        frames_gpu = []
        # slow 0.13s. TODO: move all frames to GPU on app start? vram?
        ...
        for p, f, c in zip(pred, frames_gpu, coords):  # p[3, 96, 96]. 0.003 s
            y1, y2, x1, x2 = c
            # Add a singleton dimension along axis 0
            p = p.unsqueeze(dim=0)
            if (p.shape[-1] and p.shape[-2]):
                p = F.interpolate(p, size=(int(y2 - y1), int(x2 - x1)),
                                  mode='bilinear').squeeze(dim=0).permute(1, 2, 0)  # -> [96, 96, 3]
                # Assign the processed patch to the cloned frame
                f[y1:y2, x1:x2] = p
                frames_gpu[i] = f
                i += 1
    ...
    torch.cuda.empty_cache()
    print(str(time.time())+" wav2lip done with "+args.outfile +
          " in "+str(round(time.time()-start_time, 2))+" s.")
    print(" ")
    print(" ")
